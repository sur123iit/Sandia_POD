%% ABOUT THIS CODE:
%-Applies Projected DMD to 1 run or block
%-rank reduced based on en_frac energy in Phi'Phi of svd step
%-calculates reconstruction of order N
%-contour plot of eigen vector
%-eigen value circle
%-Non-dimensional growth rate plot
%-checks effectiveness of future prediction showing contour plots
...
for jj = 1:r
    dmd_modes(:,jj) = y*V*pinv(S)*dmd_vec(:,jj)/dmd_evals(ii);
end 
dmd_modes1 = Qx*dmd_vec;
