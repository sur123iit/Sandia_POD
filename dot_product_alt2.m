...
fileName = strcat('Phi_trN_',int2str(N),'.txt');
completeName = strcat(folderName,fileName);
Phi1 = load(completeName);
fileName = strcat('Phi_trN_',int2str(N+500),'.txt');
completeName = strcat(folderName,fileName);
Phi2 = load(completeName);
phi_dot = abs(dot(Phi1(:,1:200),Phi2(:,1:200)));
plot(1:30,phi_dot(1:30),'o-','LineWidth',3);
hold on;
%%
plot([23 23],[0 phi_dot(23)],'--','LineWidth',2);
%%
