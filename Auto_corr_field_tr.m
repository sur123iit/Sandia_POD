...
count2 = 0;
for ens_num = 1:40
    for i = (ens_num-1)*N + 1 : (ens_num*N)-1
        Rij(:,1) = Rij(:,1) + (uv(:,i).*uv(:,i+1));
        count1 = count1 + 1;
    end
    for i = (ens_num-1)*N + 1 : ens_num*N
        Rij(:,2) = Rij(:,2) + (uv(:,i).^2);
        count2 = count2 + 1;
    end
end
%%
Rij(:,1) = Rij(:,1)/((N-1)*40); % Auto-correlation function
Rij(:,2) = Rij(:,2)/(N*40); % Variance 
%%
R = Rij(:,1)./Rij(:,2); % Auto-correlation coefficient
%%
